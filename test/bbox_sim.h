#ifndef RTCORE_HLS_BBOX_SIM_H
#define RTCORE_HLS_BBOX_SIM_H
...
void bbox_sim(/* in  */ hls::stream<bbox_req_t>& bbox_req_stream,
              /* out */ hls::stream<bbox_resp_t>& bbox_resp_stream) {
//#pragma HLS INTERFACE mode=ap_ctrl_none port=return
//#pragma HLS PIPELINE II=1

    bbox_req_t bbox_req;
    if (!bbox_req_stream.read_nb(bbox_req))
        return;
    preprocessed_ray_t& preprocessed_ray = bbox_req.preprocessed_ray;
    
    float& left_x_a_x = std::signbit(preprocessed_ray.w_x) ? bbox_req.left_bbox.x_max : bbox_req.left_bbox.x_min;
    float& left_x_a_y = std::signbit(preprocessed_ray.w_y) ? bbox_req.left_bbox.y_max : bbox_req.left_bbox.y_min;
    float& left_x_a_z = std::signbit(preprocessed_ray.w_z) ? bbox_req.left_bbox.z_max : bbox_req.left_bbox.z_min;
    float& left_x_b_x = std::signbit(preprocessed_ray.w_x) ? bbox_req.left_bbox.x_min : bbox_req.left_bbox.x_max;
    float& left_x_b_y = std::signbit(preprocessed_ray.w_y) ? bbox_req.left_bbox.y_min : bbox_req.left_bbox.y_max;
    float& left_x_b_z = std::signbit(preprocessed_ray.w_z) ? bbox_req.left_bbox.z_min : bbox_req.left_bbox.z_max;
    ...
    float& right_x_a_x = std::signbit(preprocessed_ray.w_x) ? bbox_req.right_bbox.x_max : bbox_req.right_bbox.x_min;
    float& right_x_a_y = std::signbit(preprocessed_ray.w_y) ? bbox_req.right_bbox.y_max : bbox_req.right_bbox.y_min;
    float& right_x_a_z = std::signbit(preprocessed_ray.w_z) ? bbox_req.right_bbox.z_max : bbox_req.right_bbox.z_min;
    float& right_x_b_x = std::signbit(preprocessed_ray.w_x) ? bbox_req.right_bbox.x_min : bbox_req.right_bbox.x_max;
    float& right_x_b_y = std::signbit(preprocessed_ray.w_y) ? bbox_req.right_bbox.y_min : bbox_req.right_bbox.y_max;
    float& right_x_b_z = std::signbit(preprocessed_ray.w_z) ? bbox_req.right_bbox.z_min : bbox_req.right_bbox.z_max;
