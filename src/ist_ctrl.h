...
//#pragma HLS INTERFACE mode=ap_ctrl_none port=return
    ...
    if (ist_ctrl_req_stream.read_nb(ist_ctrl_req)) {
        local_mem_[ist_ctrl_req.id].ray            = ist_ctrl_req.ray;
        local_mem_[ist_ctrl_req.id].num_trigs_left = ist_ctrl_req.num_trigs - 1;
        local_mem_[ist_ctrl_req.id].intersected    = 0;
        ...
        local_mem_[ist_resp.id] = local_mem;
        ...
        if (num_trigs_left == 0) {
            ...
            ist_req_t ist_req = {
                .id   = ist_resp.id,
                .ray  = local_mem.ray,
                .trig = trig_[local_mem.num_trigs_left][ist_resp.id]
            };
            ist_req_stream.write(ist_req);
        }
    }
